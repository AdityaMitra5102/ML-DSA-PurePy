class Module:
    ...
    def __repr__(self):
        return "Module over the commutative ring: {self.ring}"
        ...
        if isinstance(matrix_elements[0], list):
            for element_list in matrix_elements:
                if not all(isinstance(aij, self.ring.element) for aij in element_list):
                    raise TypeError(
                        "All elements of the matrix must be elements of the ring: {self.ring}"
                    )
...
class Matrix:
    ...
    def __repr__(self):
        ...
        info = "]\n[".join(
            [
                ", ".join(["{str(self[i, j]):>{max_col_width[j]}}" for j in range(n)])
                for i in range(m)
            ]
        )
        return "[{info}]"
