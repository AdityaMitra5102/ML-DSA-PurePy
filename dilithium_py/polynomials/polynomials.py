...
class PolynomialRingDilithium(PolynomialRing):
    ...
    def bit_unpack_s(self, input_bytes, eta):
        ...
        # Level 3 parameter set
            ...
            assert eta == 4, "Expected eta to be either 2 or 4, got {eta = }"
    ...
    def bit_unpack_w(self, input_bytes, gamma_2):
        ...
        # Level 3 and 5 parameter set
            ...
            assert gamma_2 == 261888, (
                "Expected gamma_2 to be either (q-1)/88 or (q-1)/32, got {gamma_2 = }"
            )
    ...
    def bit_unpack_z(self, input_bytes, gamma_1):
        ...
        # Level 3 and 5 parameter set
            ...
            assert gamma_1 == (1 << 19), (
                "Expected gamma_1 to be either 2^17 or 2^19, got {gamma_1 = }"
            )
    ...
    def __call__(self, coefficients, is_ntt=False):
        ...
        if not isinstance(coefficients, list):
            raise TypeError(
                "Polynomials should be constructed from a list of integers, of length at most d = {256}"
            )
        return element(self, coefficients)
...
class PolynomialDilithium(Polynomial):
    ...
    def from_ntt(self):
        raise TypeError("Polynomial is of type: {type(self)}")
    ...
    def __mul__(self, other):
        if isinstance(other, type(self)):
            ...
            raise NotImplementedError(
                "Polynomials can only be multiplied by each other, or scaled by integers, {type(other) = }, {type(self) = }"
            )
        return self.parent(new_coeffs, is_ntt=True)
