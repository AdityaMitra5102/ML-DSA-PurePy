...
class PolynomialRing:
    ...
    def __call__(self, coefficients):
        ...
        if not isinstance(coefficients, list):
            raise TypeError(
                "Polynomials should be constructed from a list of integers, of length at most d = {self.n}"
            )
        return self.element(self, coefficients)

    def __repr__(self):
        return "Univariate Polynomial Ring in x over Finite Field of size {self.q} with modulus x^{self.n} + 1"
...
class Polynomial:
    ...
    def _parse_coefficients(self, coefficients):
        ...
        if l > self.parent.n:
            raise ValueError(
                "Coefficients describe polynomial of degree greater than maximum degree {self.parent.n}"
            )
