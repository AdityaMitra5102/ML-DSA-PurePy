...
class AES256_CTR_DRBG:
    ...
    def __check_entropy_input(self, entropy_input: Optional[bytes] = None) -> bytes:
        ...
        if entropy_input is None:
            ...
            raise ValueError(
                "The entropy input must be of length: {self.seed_length}. "
                "Input has length {len(entropy_input)}"
            )
    ...
    def __instantiate(self, personalization: bytes = b"") -> bytes:
        ...
        if len(personalization) > self.seed_length:
            raise ValueError(
                "The Personalization String must be at most length: "
                "{self.seed_length}. Input has length {len(personalization)}"
            )
    ...
    def random_bytes(self, num_bytes: int, additional: Optional[bytes] = None) -> bytes:
        ...
        if additional is None:
            ...
            if len(additional) > self.seed_length:
                raise ValueError(
                    "The additional input must be of length at most: "
                    "{self.seed_length}. Input has length {len(additional)}"
                )
